...
function LocalAction({ controller, action, children }: { controller: Controller; action: ActionRequest; children: React.ReactNode }) {
  ...
  return <button type="button" disabled={reason !== null} title={formatMoneyReason(reason) ?? undefined} onClick={() => controller.queueAction(action)}>{children}</button>;
}
    ...
    <div className="world-controls">
      ...
      {(['edge', 'cache'] as const).map(kind => {
        ...
        return <div className={`world-slot ${resource ? 'installed' : 'empty'}`} key={kind} style={at(kind)} data-testid={`slot-${kind}`}>
          {!resource ? <button type="button" aria-label={kind === 'cache' ? 'Deploy Cache — reduces SQL reads' : 'Deploy Protected Edge — filters bots'} aria-disabled={controller.actionReason({ type: 'DEPLOY_RESOURCE', kind, ...tycoonPositions[kind] }) !== null} title={`${kind === 'cache' ? 'Reduces SQL reads' : 'Filters bots'} · ${definitions[kind].provisioning}s · ${formatMoneyRate(definitions[kind].cost, 'min')}`} onClick={() => { if (controller.actionReason({ type: 'DEPLOY_RESOURCE', kind, ...tycoonPositions[kind] }) === null) beginBuild(kind); }}>Build {kind === 'cache' ? 'Cache' : 'Edge'}<small>{kind === 'cache' ? 'Reads' : 'Bots'} · {definitions[kind].provisioning}s</small></button>
