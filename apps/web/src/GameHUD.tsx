...
export function GameHUD({ view }: { view: View }) {
  ...
  return <section className="game-hud" aria-label="Business status">
    <div className="hud-reading"><span aria-hidden="true">◈</span><div><small>Budget</small><strong data-testid="budget">{budget.toFixed(1)} <small>cr</small></strong><meter aria-label="Remaining operational budget" min={0} max={scenario.budget} value={Math.max(0, budget)} /></div></div>
