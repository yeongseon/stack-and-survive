...
export function MissionPanel({ view }: { view: View }) {
  return <div className="mission-panel">
    ...
    <section aria-label="Simulation event feed"><h2>Operations log</h2>
      {view.events.length === 0 ? <p className="objective-note">Build your outpost. Events appear as the system changes.</p> : <ol>{[...view.events].reverse().map(event => <li key={event.key}><time>{event.clock} {Math.floor(event.time / 60)}:{String(event.time % 60).padStart(2, '0')}</time><span>{event.text}</span></li>)}</ol>}
    </section>
  </div>;
}
