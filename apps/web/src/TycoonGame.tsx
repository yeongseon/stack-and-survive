  ...
  return <div className="tycoon-floor world" ref={host} data-testid="world" aria-label="Living cloud business">
    <div className="world-controls">
      ...
      {selected && <section className="resource-action-card" aria-label="Resource actions" onKeyDown={event => { if (event.key === 'Escape') closeCard(); }}>
        ...
        {selected.kind === 'internet' && <><p>{runtime.rateLimit ? 'Intake limited' : 'Normal intake'} · 5% of all traffic</p><LocalAction controller={controller} action={{ type: 'RATE_LIMIT', enabled: !runtime.rateLimit }}>{runtime.rateLimit ? 'Restore intake' : 'Limit intake'}</LocalAction></>}
        {selected.kind === 'edge' && <><p>Filtering {runtime.emergency ? 'boost scheduled / active' : 'normal'} · Boost costs 8 credits, lasts 30s, and rejects 3% of legitimate customers.</p><LocalAction controller={controller} action={{ type: 'EMERGENCY_WAF' }}>Boost filtering · 8 cr</LocalAction></>}
        {selected.kind === 'compute' && <><p>{app.instances} active instances. New capacity activates after 8 seconds.</p><LocalAction controller={controller} action={{ type: 'SCALE_OUT' }}>+ App capacity · +5 cr/min</LocalAction></>}
        {selected.kind === 'cache' && <p>Cache serves eligible reads. Order writes still go directly to SQL.</p>}
        {selected.kind === 'database' && <p>Read capacity 180/s · Write capacity 70/s. Cache reduces reads, not writes.</p>}
      </section>}
    </div>
  </div>;
}
...
export function TycoonGame() {
  ...
  return <main className="tycoon-game">
    {!entered ? <section className="title-screen" aria-label="Game introduction">
      ...
      <section className="tycoon-metrics" aria-label="Business status"><div>Budget<strong data-testid="budget">{view.state.economy.remainingBudget.toFixed(1)} <small>cr</small></strong></div><div>Demand<strong data-testid="traffic">{r ? r.offered.browse + r.offered.order + r.offered.bot : '—'} <small>req/s</small></strong></div><div>Availability<strong data-testid="availability">{view.snapshot ? `${(view.snapshot.metrics.availability * 100).toFixed(1)}%` : '—'}</strong></div><p role="status">{runtime.status === 'PAUSED' ? 'Paused · ' : ''}{pressure.urgent ? '⚠ ' : ''}{pressure.label}</p></section>
      ...
      {view.result && <section className="tycoon-result" aria-label="Business result"><h2>{view.result.status === 'COMPLETED' ? 'Business kept flowing' : 'Business interrupted'}</h2><p>{view.result.insight}</p><p>Score {view.result.score} · Net business value {view.result.economy.netBusinessValue.toFixed(1)} cr</p><button type="button" onClick={restart}>Play again</button><button type="button" onClick={() => open('learn')}>Review business</button></section>}
