...
it('explains all core terms without requiring hover or sound', () => {
  expect(glossary.map(([term]) => term)).toEqual(['Pressure markers', 'RPS', 'Availability', 'Latency', 'Utilization', 'Cache hit ratio', 'Scale out', 'WAF', 'Rate Limit', 'Net business value', 'Upgrade Funds']);
});
