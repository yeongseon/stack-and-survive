...
function World({ controller, generation, onInspect }: { controller: Controller; generation: number; onInspect: (open: boolean) => void }) {
  ...
  useEffect(() => {
    ...
    mountWorld(surface, controller, generation, () => onInspect(true)).then(destroy => { if (disposed) destroy(); else cleanup = destroy; })
  ...
  }, [controller, generation, onInspect]);
...
function App() {
  ...
  return <main className="viewport-shell" ref={shell}>
    ...
    <header><div className="game-brand"><span className="game-mark" aria-hidden="true">S<span>+</span></span><div><h1>STACK <span>&amp;</span> SURVIVE</h1><p className="eyebrow">BUILD. SCALE. KEEP THE BUSINESS FLOWING.</p></div></div><div className="scenario"><span className="scenario-title">BLACK FRIDAY <small>OPERATION 01</small></span><strong data-testid="status">{view.error ? 'STOPPED · ERROR' : inspecting && running ? 'MANUAL INSPECTION' : view.state.runtime.status}</strong><span><b data-testid="elapsed">{view.state.runtime.time}</b> / 180 seconds</span></div></header>
    <section id="scenario-controls" tabIndex={-1} className="controls" aria-label="Scenario controls">
      ...
      <button type="button" disabled={!preparing || !!view.error || readyErrors.length > 0} onClick={() => controller.start()}>Start operation</button>
      <button type="button" disabled={paused || !!view.error} onClick={() => { controller.reset(); setInspecting(false); }}>Reset baseline</button>
      <button type="button" disabled={(!running && !paused) || !!view.error} onClick={() => { if (paused) controller.resume(); else controller.pause(); setInspecting(false); }}>{paused ? 'Resume operation' : 'Pause operation'}</button>
      <button type="button" aria-expanded={buildOpen} aria-controls="build-panel" onClick={() => setBuildOpen(!buildOpen)}>Build &amp; connections</button>
      <p>Real simulation · 1 tick / second · Representative traffic, not one sprite per request</p>
    </section>
    <details className="help"><summary>How to play &amp; glossary</summary><p>Your application is online. Business demand is growing. Keep customers served and operating costs under control.</p><ol><li>Select a resource to inspect its role and cost.</li><li>Place and connect resources before starting; wait for required provisioning.</li><li>Start operation, observe flow pressure, and expand or optimize deliberately.</li><li>Review the result, redesign, and retry the same workload.</li></ol><p>Click or tap controls; the resource list below also supports keyboard selection. Drag buildings to move, drag empty ground to pan, and scroll to zoom. Pause is for inspection, not free recovery. No sound is required.</p><dl>{glossary.map(([term, definition]) => <div key={term}><dt>{term}</dt><dd>{definition}</dd></div>)}</dl></details>
    <section aria-label="Local architecture save" className="save-controls"><button type="button" onClick={() => controller.saveArchitecture()}>Save architecture</button><button type="button" disabled={!preparing} onClick={() => controller.clearLocalState()}>Clear local state</button><p role="status" data-testid="save-status">{view.saveMessage}</p><p>Only the architecture is saved. Refresh returns to Build Mode, cancels unfinished scale-out and restarts unfinished resource deployment. Scenario progress is not saved.</p></section>
    ...
    <section className={`live-actions${actionDetails ? ' expanded' : ''}`} aria-label="Live interventions">
    ...
    <button className="action-help-toggle" type="button" aria-expanded={actionDetails} onClick={() => setActionDetails(!actionDetails)}>{actionDetails ? 'Hide action explanations' : 'Action costs & explanations'}</button>
