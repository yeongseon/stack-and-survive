...
function App() {
  ...
  const instances = view.state.runtime.architecture.resources.find(n => n.kind === 'compute')!.instances;
  return <main>
    ...
    <section className="controls" aria-label="Scenario controls">
      ...
      <button type="button" disabled={view.state.runtime.status !== 'PREPARATION' || !!view.error} onClick={() => controller.start()}>Start traffic</button>
