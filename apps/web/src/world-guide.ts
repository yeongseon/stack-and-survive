...
export function guideHint(view: View, stage: GuideStage): GuideHint {
  ...
  if (compare(view.state.economy.remainingBudget / (view.challenge?.workload.budget ?? blackFriday.budget), .2) < 0) return { title: 'Budget is running low', text: 'Check running costs before expanding. Earned revenue does not refill this operational budget.', target: null };
  ...
  if (compare(r.sql.readUtilization, 1) > 0 && !resources.some(resource => resource.kind === 'cache')) return { title: 'Consider reducing SQL reads', text: 'The Cache pad shows its running cost and build delay. Eligible reads can finish there; Order writes still need SQL. This is an option, not a required answer.', target: 'cache' };
  ...
  if (compare(r.app.utilization, .7) > 0 && app && app.instances < 4) return { title: 'Consider more App capacity', text: 'Open the highlighted App pad to inspect the cost. A new server takes eight seconds to activate; you can cancel the confirmation or choose another approach.', target: 'compute' };
