...
export function guideHint(view: View, stage: GuideStage): GuideHint {
  ...
  if (compare(view.state.economy.remainingBudget / blackFriday.budget, .2) < 0) return { title: 'Budget is running low', text: 'Check running costs before expanding. Earned revenue does not refill this operational budget.', target: null };
