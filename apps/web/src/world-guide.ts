...
export function guideHint(view: View, stage: GuideStage): GuideHint {
  ...
  if (stage === 'observe') return { title: 'Follow the traffic', text: 'Customers enter at the Internet intake (left). Requests flow through App, then to SQL. Watch where packets slow down or drop.', target: 'internet' };
