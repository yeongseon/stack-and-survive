...
The image above is a refreshed **rules 0.4 / `a6f6956`** local production capture. [Provenance](images/README.md) records its source/build and automation limits. [Current status](CURRENT_STATUS.md) separates merged AI code from live activation.
...
Current screenshot workflow: `node scripts/capture-current-media.mjs` builds API-disabled production and records ordinary play; `node scripts/publish-current-images.mjs <capture-directory>` converts the verified 19-image set. The personal story renderer is now separate: `node scripts/render-project-video.mjs --voice` renders the same eight slides/story as the PDF, or use `--narration-dir=/absolute/path` for your own per-slide WAV recordings. With no audio option it emits a silent captioned version. `node scripts/check-project-media.mjs` verifies deck/image/video hashes, exact duration and playback. [Instructions](DEMO_VIDEO.md#automatic-regeneration). No gameplay or model response is fabricated.
