...
The next phase is described, not always called a wave: the last scheduled phase is FINAL WAVE; lower RPS with non-increasing bot RPS is Recovery window; increased bot RPS is Bot attack; otherwise increased RPS is Traffic spike (unchanged load: Traffic change). These labels derive from existing phase data; they do not change balance or promise that every architecture recovers. The last5s emphasis and paused qualifiers remain.
...
The per-tick Orders served banner is removed so request/activation/recovery cues are not drowned out. Funds/lost sales remain in HUD, detailed revenue in Learn/results. Optional sound/vibration stays default-off and gesture-enabled; [audio/manual acceptance](AUDIO_FEEDBACK.md) remains separate. The skippable [world guide](WORLD_GUIDE.md) never acts or pauses for the player. Tips are not a comprehension score.
...
The title has a secondary Settings button. Player Pause or Escape during normal operation opens a native modal menu with Resume, Settings, How to Play, Inspect paused world and Return to Title. Inspect closes the menu without resuming, allowing keyboard/camera/resource inspection; Escape reopens the pause menu. Escape on Pause resumes; Escape in Settings returns to Pause, or closes title Settings. Existing Learn remains available outside the menu and does not silently change the run clock.

Settings reuses sound/mute/volume, supported vibration and guide preferences; fullscreen enter/exit is optional and failures disclose windowed play. Phone orientation hints retain the existing landscape gate. Closing Settings/help does not resume paused gameplay. Nested How to Play/About returns to Pause when opened there. A paused menu interrupted by portrait rotation restores after explicit landscape Continue without unpausing. Renderer error dismisses menus/help and focuses Rebuild graphics. Restart cleans menu state and restores title Start focus. Reduced motion removes the short menu fade; all buttons remain touch-sized. No new persistence or simulation state is introduced.
