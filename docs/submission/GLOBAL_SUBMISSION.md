...
The deck and 120-second video open with a two-second Stack & Survive title cover, followed by eight personal story slides (nine PDF pages). Approved GuyNeural speech is unchanged; the first spoken section uses 14 seconds rather than 16 to preserve the full two-minute length. Actual screenshots support questions and reflection, with no AI segment, product pitch or production banner. Agent details are Q&A only. The voice is disclosed synthetic narration, not a human recording. No learning effect or live AI service is claimed.
...
- [x] Automated: cover plus eight slides at 1920×1080, 1440×900, 1366×768 and 820×1180; navigation, images, links, print, no-JavaScript order and forbidden labels. This is not human delivery validation.
- [ ] Check all nine PDF pages and browser controls on the actual presentation device.
