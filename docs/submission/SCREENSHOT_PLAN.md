...
Current 19-image walkthrough uses `scripts/capture-current-media.mjs` and the separate `scripts/publish-current-images.mjs`; [provenance](../images/README.md) records readouts/hashes. The eight-slide personal video consumes those verified images without changing them. The older `capture-submission.mjs` procedure below is historical and uses legacy selectors/storage; `capture-tradeoff.mjs` remains a separate technical recording.
