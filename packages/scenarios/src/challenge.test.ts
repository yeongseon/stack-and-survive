...
import { blackFridayChallenge, parseChallenge, sameChallenge } from './challenge';
...
it('canonicalizes key ordering but distinguishes every condition used for comparisons', () => {
  ...
  const reordered = parseChallenge({ workload: { ...blackFriday, targets: { netBusinessValue: 360, latencyMs: 300, availability: .99 } },
    objective: { kind: 'survive', id: 'survive', version: 1 }, rulesVersion: '0.2', seed: 0, seedAlgorithm: 'fixed-v1', version: 1, id: 'black-friday', schemaVersion: 1 });
