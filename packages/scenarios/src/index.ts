...
export function parseScenario(input: unknown): Scenario {
  ...
  if (s.schemaVersion !== 1 || (s.balanceVersion !== '0.2' && s.balanceVersion !== '0.3')) throw new Error('Unsupported scenario/balance version');
  ...
  return Object.freeze({ schemaVersion: 1, balanceVersion: s.balanceVersion, id: text(s.id, 'id'), duration, budget: number(s.budget, 'budget', Number.MIN_VALUE),
