...
import legacyData from './black-friday-v02.json';

export function parseScenario(input: unknown): Scenario {
  const s = record(input, 'scenario');
  if (s.schemaVersion !== 1 || !['0.2', '0.3', '0.4'].includes(String(s.balanceVersion))) throw new Error('Unsupported scenario/balance version');
  ...
  return Object.freeze({ schemaVersion: 1, balanceVersion: s.balanceVersion as Scenario['balanceVersion'], id: text(s.id, 'id'), duration, budget: number(s.budget, 'budget', Number.MIN_VALUE),
...
export const blackFridayV02 = parseScenario(legacyData);
export const infrastructureScalingScenario = parseScenario({ ...data, balanceVersion: '0.4' });
