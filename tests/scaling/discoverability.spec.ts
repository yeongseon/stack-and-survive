...
for (const width of [1440, 667]) {
  test(`visible scaling controls work with ${width === 667 ? 'touch' : 'mouse'} at ${width}`, async ({ page }, info) => {
    ...
    await expect(card.getByRole('button', { name: 'Scale down · ↓ SQL tier', exact: true })).toBeDisabled();
    await activate('Scale up · ↑ SQL tier');
    ...
    await activate('Scale down · ↓ SQL tier');
    await expect(card).toContainText('Tier 1 · General Purpose I');
    await activate('Ⅱ Pause'); await activate('Inspect paused world');
    await expect(card.getByRole('button', { name: 'Scale up · ↑ SQL tier', exact: true })).toBeDisabled();
    await expect(card.getByRole('button', { name: 'Scale up · ↑ SQL tier', exact: true })).toHaveAccessibleDescription(/running|paused/i);
